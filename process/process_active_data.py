...
if __name__ == '__main__':
    ...
    for layer_num in [3, 5, 10]:
        model = load_model('../model/mnist/mnist_' + str(layer_num) + '_hidden_layers_model.hdf5')
        layers = get_activation_layers(model)
        datas = []
        datas_by_sample = []
        # train_datas, train_labels, test_datas, test_labels = data_provider.get_mnist_data()
        attack_datas = np.load(
            '../data/mnist/mnist_attack_data/' + str(layer_num) + '_hidden_layers_model_attack_datas.npy')
        ...
        save_dir_path = os.path.join(config.data_save_path, 'mnist/mnist_attack_active_data')
